...
export async function generateCodePrompt(cwd: string): Promise<string> {
...
- Leverage the \`search_files\` tool in combination with other tools for more comprehensive analysis. For example, use it to find specific code patterns, then use \`read_file\` to examine the full context of interesting matches before using \`agentic_apply_diff\` or \`write_to_file\` to make informed changes.
- For editing files, you have access to these tools: \`agentic_apply_diff\` (for surgical edits - targeted changes to specific lines or functions), \`write_to_file\` (for creating new files or complete file rewrites), \`insert_content\` (for inserting lines to files at specific line numbers).
- \`The insert_content\` tool adds lines of text to files at a specific line number, such as adding a new function to a JavaScript file or inserting a new route in a Python file. Use line number 0 to append at the end of the file, or any positive number to insert before that line.
- You should use \`insert_content\` tool only when you need to add new lines to a file without modifying existing content. For modifying existing content, prefer using \`agentic_apply_diff\` tool.
...
Home Directory: C:/Users/tt887
Current Workspace Directory: c:/Users/tt887/Dev/Other/kilocode
