...
export class AgenticDiffApplyTool extends BaseTool<"agentic_apply_diff"> {
	...
	async execute(params: AgenticDiffApplyParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const newTask = await task.startSubtask(unescapedDiffblocks, emptyTodoItems, "diff_apply")
			...
			pushToolResult(`Successfully created new task in diff_apply mode with diffblocks`)
